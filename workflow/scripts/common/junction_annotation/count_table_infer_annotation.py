...
df_annotation = df_annotation.set_index('gene_id').join(coding_genes.set_index('gene_id')[['gene_name', 'gene_type']]).reset_index()
df_annotation[
    df_annotation['gene_type'] == 'protein_coding'
]
assert df_annotation['gene_id'].apply(lambda x: x.split(';')).map(len).max() == 1
